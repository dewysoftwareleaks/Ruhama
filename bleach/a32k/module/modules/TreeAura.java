...
public class TreeAura extends Module
{
        ...
        } else
        {
            ...
            if (this.getSettings().get(4).toToggle().state)
            {
                    ...
                    for (y = -1; y <= 1; ++y)
                    {
                        for (z = -2; z <= 1; ++z)
                        {
                            ...
                            if ((block == Blocks.GRASS || block == Blocks.DIRT) && (block1 == Blocks.GRASS || block1 == Blocks.DIRT) && (block2 == Blocks.GRASS || block2 == Blocks.DIRT) && (block3 == Blocks.GRASS || block3 == Blocks.DIRT) && this.isBlockAir(this.pos) && this.isBlockAir(this.pos.east()) && this.isBlockAir(this.pos.south()) && this.isBlockAir(this.pos.south().east()) && this.canTreeGrow(this.pos))
                            {
                                ...
                                {
                                    this.queue.add(new TreeAura.WaitAction((int) this.getSettings().get(5).toSlider().getValue()));
                                }

                                this.queue.add(new TreeAura.PlaceAction(this.pos, sapling));
                                this.queue.add(new TreeAura.PlaceAction(this.pos.east(), sapling));
                                this.queue.add(new TreeAura.PlaceAction(this.pos.south(), sapling));
                                this.queue.add(new TreeAura.PlaceAction(this.pos.south().east(), sapling));
                                return;
                            }
                ...
                if (backupPos != null)
                {
                    ...
                    this.queue.add(new TreeAura.PlaceAction(this.pos.down(), dirt));
                    this.queue.add(new TreeAura.PlaceAction(this.pos.down().east(), dirt));
                    this.queue.add(new TreeAura.PlaceAction(this.pos.down().south(), dirt));
                    this.queue.add(new TreeAura.PlaceAction(this.pos.down().south().east(), dirt));
                    this.queue.add(new TreeAura.WaitAction(2));
                    this.queue.add(new TreeAura.PlaceAction(this.pos, sapling));
                    this.queue.add(new TreeAura.PlaceAction(this.pos.east(), sapling));
                    this.queue.add(new TreeAura.PlaceAction(this.pos.south(), sapling));
                    this.queue.add(new TreeAura.PlaceAction(this.pos.south().east(), sapling));
    ...
    public void onUpdate()
    {
        if (this.pos != null && this.mc.player.getDistance((double) this.pos.getX() + 0.5D, (double) this.pos.getY() + 0.5D, (double) this.pos.getZ() + 0.5D) <= 4.5D && this.timesBonemealed <= 10 && this.mc.world.getBlockState(this.pos).getBlock() != Blocks.LOG && this.mc.world.getBlockState(this.pos).getBlock() != Blocks.LOG2)
        {
            ...
            } else
            {
                ...
                {
                    this.queue.add(0, new TreeAura.WaitAction(1));
                }

                if (this.queue.isEmpty() && this.getSettings().get(1).toToggle().state && bonemeal != -1 && this.timesBonemealed < 10)
                {
                    this.queue.add(new TreeAura.BonemealAction(this.pos, bonemeal));
    ...
    public void processQueue()
    {
        if (!this.queue.isEmpty())
        {
            ...
            } else if (this.queue.get(0) instanceof TreeAura.PlaceAction)
            {
                ...
                {
                    this.queue.add(0, new TreeAura.WaitAction((int) this.getSettings().get(6).toSlider().getValue()));
                }
    ...
    class BonemealAction extends TreeAura.TreeAction
    {
        public int slot;
    ...
    class PlaceAction extends TreeAura.TreeAction
    {
        public int slot;
    ...
    class WaitAction extends TreeAura.TreeAction
    {
